...
public class Sierpinski {
    ...
    private static void usage() {
        System.out.println("usage: java -jar sierpinski.jar [ -depth 3 ] [ -length 1.0 ] [ sierpinski.stl ]");
    }
    ...
    public static void main(String[] args) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        if (args.length == 0) {
            ...
            double baseLength = 1d;
            int recursionDepth = 3;
            File outputFile = new File("sierpinski.stl").getAbsoluteFile();
            
            for(int i=0; i<args.length; i++) {
                if ("-help".equals(args[i])) {
                    usage(); System.exit(0);
                } else if ("-depth".equals(args[i])) {
                    recursionDepth = Integer.parseInt(args[++i]);
               } else if ("-length".equals(args[i])) {
                    baseLength = Double.parseDouble(args[++i]);
                } else if (args[i].startsWith("-") == false && i == args.length - 1) {
                    outputFile = new File(args[i]).getAbsoluteFile();
                }
            }
            
            generateFractal(recursionDepth, baseLength, outputFile);
        }
    }
