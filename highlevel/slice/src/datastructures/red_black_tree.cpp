...
rb_red_blk_node * RBTreeInsert(rb_red_blk_tree* tree, void* key, void* info) {
  ...
  if (tree->nil == tree->min || (1 == tree->Compare(tree->min->key, newNode->key)) ) {
    tree->min = newNode;
  }
  return(newNode);
...
/* our functions */
int rb_findsucc(int i, rb_red_blk_tree* tree) {
  rb_red_blk_node* nil=tree->nil;
  rb_red_blk_node* x=tree->root->left;
  rb_red_blk_node* lastBest=nil;
  int *ip = &i;
  while(nil != x) {
    if ( -1 == (tree->Compare(x->key,ip)) ) { /* x->key < i */
      x=x->right;
    } else {
      lastBest=x;
      x=x->left;
  ...
  if (lastBest == nil )
    return -1;
  return *((int*)lastBest->key);
}

int rb_findpred(int i, rb_red_blk_tree* tree) {
  rb_red_blk_node* nil=tree->nil;
  rb_red_blk_node* x=tree->root->left;
  rb_red_blk_node* lastBest=nil;
  int *ip = &i;
...
rb_red_blk_node * RBFindMin(rb_red_blk_tree* tree) {
  return (tree->nil != tree->min) ? tree->min : NULL;
}
rb_red_blk_node *RBDeleteMin(rb_red_blk_tree* tree) {
  rb_red_blk_node *tmp = RBFindMin(tree);
  if (NULL != tmp) 
    RBDelete(tree, tmp);
  return tmp;
}

void RBDecreaseKey(rb_red_blk_tree* tree, rb_red_blk_node *node, int val) {

  int *newKey = (int *) malloc(sizeof(int*));
  // void *info = (void *) malloc(sizeof(void*));
  ...
  *newKey -= val;
  ...
  RBDelete(tree, node);
  RBTreeInsert(tree, newKey, NULL);
}

rb_red_blk_node * RBMakeNode(int key) {
  rb_red_blk_node* temp;
  int *newInt;
  newInt = (int*) malloc(sizeof(int));
  *newInt = key;

  temp = (rb_red_blk_node*) SafeMalloc(sizeof(rb_red_blk_node));
  temp->parent = temp->left = temp->right=0;
  temp->red = 0;
  temp->key = newInt;
  return temp;
}

rb_red_blk_node * RBTreeInsert_(rb_red_blk_tree* tree, rb_red_blk_node** n) {
   RBTreeInsert(tree, (*n)->key, NULL);
  return *n;
}
